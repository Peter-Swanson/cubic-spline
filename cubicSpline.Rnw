...
In general, a \textbf{spline} is defined as a degree $d$ piecewise polynomial whose function values and first $d-1$ derivatives agree at the points where they join (knots) \cite{hastie2011elements, smith1979splines, devlin1986spline, james2013introduction, harrell2015regression}. Splines are used to fit non-linear relationships between $X$ and $Y$ and are:
...
The important thing here is that the linear model in \ref{simpLinMod} imposes a linear relationship. It is linear in the coefficients, $\beta$, not necessarily in $X$. So if $X$ does not have a linear relationship with $Y$, we would be violating a basic model assumption to use \ref{simpLinMod} to fit the relationship between $X$ and $Y$.
